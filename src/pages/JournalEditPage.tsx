...
export function JournalEditPage() {
  ...
  useEffect(() => {
    if (readOnly || !form.formState.isDirty) return
  ...
  }, [watchedValues, form.formState.isDirty, readOnly])
  ...
  const markTopics = async () => {
    ...
    try {
      await Promise.all(topics.filter(({ item }) => selected.has(item.id)).map(({ item, itemType }) => roadmap.setProgressField(itemType, item.id, { learnerCompleted: true, learnerCompletedBy: learnerId })))
  ...
  return (
    <>
      <PageHeader title={readOnly ? 'Eingereichter Wochenrückblick' : 'Wochenrückblick'} description={readOnly ? 'Eingereichte Rückblicke können nicht verändert werden.' : 'Reflektiere deine Lernwoche Schritt für Schritt.'} />
      ...
      <Card>
        <div className="mb-6 grid gap-4 sm:grid-cols-2">
          ...
          <div><Label htmlFor="week-date">Woche anpassen</Label><Input id="week-date" type="date" value={Number.isNaN(weekDate.getTime()) ? '' : weekDate.toISOString().slice(0, 10)} disabled={readOnly} onChange={(event) => updateWeek(event.target.value)} /></div>
          <div><Label>Zeitraum</Label><p className="mt-1 text-sm text-ink-muted">{new Date(form.watch('weekStart')).toLocaleDateString('de-CH')} – {new Date(form.watch('weekEnd')).toLocaleDateString('de-CH')}</p></div>
          <div><Label>Semester</Label><p className="mt-1 text-sm">{activeSemester?.label ?? 'Früheres Semester'}</p></div>
        </div>
        <form onSubmit={submit}>
          <JournalForm form={form} topics={topics as RoadmapItemView[]} readOnly={readOnly} />
          {!readOnly ? <div className="mt-6 flex flex-wrap gap-3"><Button type="button" variant="outline" onClick={() => void save()}>Entwurf speichern</Button><Button type="submit">Einreichen</Button></div> : null}
        </form>
      </Card>
